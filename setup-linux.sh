...
source "$SCRIPT_DIR/scripts/linux/setup-common.sh"
...
repo_package_script_rel() {
  ...
  case "$repo" in
  zenmind-gateway | zenmind-app-server | mcp-server-mock | mcp-server-bash | mcp-server-email)
    printf 'release-scripts/linux/package.sh\n'
    ;;
  *)
    return 1
    ;;
...
repo_start_script_abs() {
  local repo="$1"
  printf '%s/release-scripts/linux/start.sh\n' "$(repo_release_dir "$repo")"
}

repo_stop_script_abs() {
  local repo="$1"
  printf '%s/release-scripts/linux/stop.sh\n' "$(repo_release_dir "$repo")"
}
...
print_summary() {
  ...
  if ((${#SUMMARY_FAIL[@]} > 0)); then
    ...
    cat <<'HINT' >&2
[setup-linux] common fix hints:
  - Run precheck first: ./setup-linux.sh --action precheck
  - Install dependencies (Ubuntu/WSL baseline): sudo apt-get update && sudo apt-get install -y git openjdk-21-jdk maven apache2-utils python3
  - Install Go 1.26.0+ and Node.js 20+ with your preferred version manager or vendor packages
  - Ensure Go version is 1.26.0 or newer
  - Verify each service repo has release-scripts/linux/package.sh plus release-scripts/linux/start.sh and stop.sh
HINT
  fi
}

ensure_check_script_ready() {
  local check_script="$SCRIPT_DIR/scripts/linux/check-environment.sh"
...
check_required_repo_files() {
  local failed=0
  local repo package_script start_script stop_script
  ...
  for repo in "${MANAGED_REPOS[@]}"; do
    ...
    start_script="release-scripts/linux/start.sh"
    stop_script="release-scripts/linux/stop.sh"
    check_repo_file "$repo" "$package_script" || failed=1
    check_repo_file "$repo" "$start_script" || failed=1
    check_repo_file "$repo" "$stop_script" || failed=1
  done

  return "$failed"
}

check_existing_repo_contracts_for_precheck() {
  local present=0
  local failed=0
  local repo source_dir

  for repo in "${MANAGED_REPOS[@]}"; do
    source_dir="$(repo_source_dir "$repo")"
    if [[ -d "$source_dir" ]]; then
      present=1
      check_repo_file "$repo" "README.md" || failed=1
      check_repo_file "$repo" "release-scripts/linux/package.sh" || failed=1
      check_repo_file "$repo" "release-scripts/linux/start.sh" || failed=1
      check_repo_file "$repo" "release-scripts/linux/stop.sh" || failed=1
    fi
  done

  if [[ "$present" == "0" ]]; then
    summary_add_warn "source repos not cloned yet, skip linux release script contract checks"
    return 0
  fi
...
write_startup_config() {
  ...
  {
    printf '# Managed by setup-linux.sh\n'
...
configure_startup_services_interactive() {
  ...
  for repo in "${DEFAULT_STARTUP_ORDER[@]}"; do
    prompt="[setup-linux] enable ${repo}? [Y/n]: "
...
run_precheck() {
  ...
  check_existing_repo_contracts_for_precheck || failed=1
  return "$failed"
}
...
start_repo() {
  ...
  if [[ "$running_count" != "0" ]]; then
    ...
    if (cd "$release_repo" && ./release-scripts/linux/stop.sh); then
      :
    else
  ...
  setup_log "starting $repo via release-scripts/linux/start.sh"
  if (cd "$release_repo" && ./release-scripts/linux/start.sh); then
...
main() {
  ...
  if [[ "$(uname -s)" != "Linux" ]]; then
    setup_err "setup-linux.sh must be run on Linux or inside WSL"
    setup_err "for macOS use: ./setup-mac.sh"
    exit 1
  fi
