...
#
# Optional env:
#   AUTO_INSTALL=1            # auto-install nginx on Ubuntu/Debian via apt-get
#   DOMAIN=app.zenmind.cc      # if you want server_name; default "_"
#   LISTEN_IP=127.0.0.1
#   LISTEN_PORT=11945
#   OS_NAME=linux              # optional override
...
ensure_nginx_exists() {
  ...
  if [[ "${AUTO_INSTALL}" == "1" ]]; then
    if [[ "${OS_NAME}" != "linux" ]]; then
      echo "ERROR: AUTO_INSTALL=1 currently supports Linux only." >&2
      echo "Install nginx manually, then rerun this script."
      exit 1
    fi
    if ! command -v apt-get >/dev/null 2>&1; then
      echo "ERROR: nginx missing and apt-get not found. Install nginx manually for your distro." >&2
      exit 1
    fi
    echo "nginx not found in PATH. Installing via apt-get because AUTO_INSTALL=1..."
    if [[ "${EUID}" == "0" ]]; then
      apt-get update
      ...
      echo "ERROR: sudo not found. Re-run as root or install nginx manually." >&2
      exit 1
    fi
  else
    echo "ERROR: nginx command not found in PATH."
    echo "Fix PATH or install nginx. (Set AUTO_INSTALL=1 to auto-install via apt-get on Ubuntu/Debian.)"
    exit 1
  fi
}
...
- Start nginx:   ./scripts/linux/start-nginx.sh
- Stop nginx:    ./scripts/linux/stop-nginx.sh
- Restart nginx: ./scripts/linux/restart-nginx.sh
EOF
}
