...
setup_check_docker_compose_for_app_server() {
  ...
  if ! docker compose version >/dev/null 2>&1; then
    ...
    setup_err "if using podman, install podman-compose and start the podman service or machine"
    return 1
  fi
  ...
  if ! setup_docker_daemon_running; then
    setup_warn "docker runtime not ready (if using podman alias: start the podman service or machine)"
  fi
...
setup_check_optional_tools() {
  if command -v htpasswd >/dev/null 2>&1; then
    ...
    setup_warn "optional tool missing: htpasswd (Ubuntu/WSL install hint: sudo apt-get install -y apache2-utils)"
  fi
...
setup_prompt_password() {
  ...
  if [[ "${SETUP_NON_INTERACTIVE:-0}" == "1" ]]; then
    ...
    if [[ "$show_plain_password" == "1" ]]; then
      read -r -p "[setup-linux] $prompt [default: $default_password]: " input
    else
      read -r -s -p "[setup-linux] $prompt [default: $default_password]: " input
    fi
...
setup_show_first_install_password_notice() {
  ...
  cat <<'NOTICE'
[setup-linux] 首次安装将要求输入 3 组密码（后续会自动加密写入配置）:
[setup-linux] 1) term-webclient: AUTH_PASSWORD_HASH_BCRYPT 对应明文密码
[setup-linux] 2) zenmind-app-server: AUTH_ADMIN_PASSWORD_BCRYPT 对应明文密码
[setup-linux] 3) zenmind-app-server: AUTH_APP_MASTER_PASSWORD_BCRYPT 对应明文密码
[setup-linux] First install will prompt for 3 passwords (hashed and written to config):
[setup-linux] 1) term-webclient: plaintext password for AUTH_PASSWORD_HASH_BCRYPT
[setup-linux] 2) zenmind-app-server: plaintext password for AUTH_ADMIN_PASSWORD_BCRYPT
[setup-linux] 3) zenmind-app-server: plaintext password for AUTH_APP_MASTER_PASSWORD_BCRYPT
[setup-linux] 按回车继续 / Press Enter to continue...
NOTICE
  read -r _
}
