...
[[ -f "$CRED_FILE" ]] || die "未找到 tunnel 凭据文件: $CRED_FILE。请先创建 tunnel，或运行 ./scripts/linux/create-cf-tunnel-config-sysadmin.sh"
...
if [[ "$START_NOW" =~ ^[Yy]$ ]]; then
  echo "启动：./scripts/linux/start-cf-tunnel.sh --foreground"
  exec "$(cd "$(dirname "$0")" && pwd)/start-cf-tunnel.sh" --foreground
else
  echo "手动启动（后台）：./scripts/linux/start-cf-tunnel.sh"
  echo "停止命令：./scripts/linux/stop-cf-tunnel.sh"
fi
